...
def run_verification():
    with sync_playwright() as p:
        ...
        try:
            ...
            # Enter a prompt that should trigger the correlation matrix function
            page.get_by_placeholder("Faça uma pergunta sobre o arquivo...").fill("mostre a matriz de correlação")
            page.keyboard.press("Enter")

            # Wait for the final assistant's response, which should mention the matrix
            assistant_response = page.locator("div[data-testid='stChatMessage']").last
            expect(assistant_response).to_contain_text("matriz de correlação", timeout=60000)

            # Check if a plot was generated (heatmap is an image)
