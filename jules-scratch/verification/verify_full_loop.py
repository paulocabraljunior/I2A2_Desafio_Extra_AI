...
def run_verification():
    with sync_playwright() as p:
        ...
        try:
            ...
            # Wait for the final assistant's response
            # This part will fail without an API key, but it validates the full loop
            assistant_response = page.locator("div[data-testid='stChatMessage']").last
            expect(assistant_response).to_contain_text("alpha", timeout=60000)
            expect(assistant_response).to_contain_text("beta")
